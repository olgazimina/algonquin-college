...
import cst8284.asgmt4.exceptions.BadRoomBookingException;
import cst8284.asgmt4.room.Room;
import cst8284.asgmt4.views.RoomBookingDialog;
...
public class RoomScheduler {
	...
	/**
	 * Starts the main logic and main window
	 */
	public void launch() {
		// preload data
		getRoomBookings().addAll(loadBookingsFromFile("CurrentRoomBookings.book"));
		new RoomSchedulerDialog().run(this);
	}
	 ...
	 * @param bookingInput data from dialog window
	 */
	public String executeMenuItem(int choice, BookingInput bookingInput) {
		...
		switch (choice) {
			...
			case ENTER_ROOM_BOOKING:
				if (saveRoomBooking(makeBookingFromUserInput(bookingInput))) {
					return "Booking time and date saved";
				} else {
					return "Cannot save booking; that time is already booked";
				}

				// when user decided to delete booking
			case DELETE_BOOKING:
				// we need to call "deleteBooking" with a parameter of newly created Calendar
				if (deleteBooking(makeCalendarFromUserInput(null, true, bookingInput))) {
					return "Booking successfully deleted";
				} else {
					return "There is no booking starting at that time";
				}

				// in case user wants to change booking
			case CHANGE_BOOKING:
				// we need to call "changeBooking()" method with a parameter of newly created Calendar
				BookingInput oldBooking = new BookingInput(bookingInput.getEventClient(), bookingInput.getEventPhoneNum(), bookingInput.getOldEventDate(),
					bookingInput.getOldEventSTime(), bookingInput.getEventETime(), bookingInput.getEventDescription(),
					bookingInput.getEventCategory(), bookingInput.getEventOrganization());

				if (this.changeBooking(makeCalendarFromUserInput(null, true, oldBooking), bookingInput)) {
					return "Booking has been successfully changed to: \n" + displayBooking(null, makeCalendarFromUserInput(null, true, bookingInput));
				} else {
			...
			case DISPLAY_BOOKING:
				...
				Calendar calendar = makeCalendarFromUserInput(null, true, bookingInput);
				return displayBooking(findBooking(calendar), calendar);
			...
			case DISPLAY_DAY_BOOKINGS:
				...
				return displayDayBookings(makeCalendarFromUserInput(null, false, bookingInput));
			...
			case SAVE_BOOKINGS_TO_FILE:
				...
				if (this.saveBookingsToFile(bookingInput.getFile())) {
					return "Current room bookings backed up to file";
				} else {
					return "Bookings cannot be saved to file";
				}
			...
			case LOAD_BOOKINGS_FROM_FILE:
				...
				ArrayList<RoomBooking> loadedRoomBookings = this.loadBookingsFromFile(bookingInput.getFile());
				...
				if (loadedRoomBookings.size() > 0) {
					...
					this.roomBookings = this.loadBookingsFromFile(bookingInput.getFile());
				} else {
					// otherwise we need to print error message
					return "There is no bookings in the file or file is missing/corrupted";
				}
				break;
			...
			case EXIT:
				...
				this.saveBookingsToFile(bookingInput.getFile());
			default:
				return "";
		}
		return "";
	 ...
	 */
	private boolean changeBooking(Calendar calendar, BookingInput bookingInput) {
		RoomBooking roomBooking = findBooking(calendar);
		...
		int      hour      = processTimeString(checkIfNullOrEmpty(bookingInput.getEventSTime()));
		Calendar startTime = makeCalendarFromUserInput(null, true, bookingInput);
		...
		Calendar endTime = makeCalendarFromUserInput(startTime, true, bookingInput);

		// havinf start time and end time we can setup new time block instance
		TimeBlock timeBlock = new TimeBlock(startTime, endTime);

		RoomBooking newBooking = makeBookingFromUserInput(bookingInput);
		...
		if (checkBooking == null || roomBooking.equals(checkBooking)) {
			...
			getRoomBookings().get(roomBookingIndex).setActivity(newBooking.getActivity());
			getRoomBookings().get(roomBookingIndex).setContactInfo(newBooking.getContactInfo());
	 ...
	 */
	private String displayBooking(RoomBooking booking, Calendar calendar) {
		return booking != null ?
			"---------------\n" + booking.toString() + "---------------\n" :
	 ...
	 */
	private String displayDayBookings(Calendar cal) {
		...
		for (int hrCtr = 8; hrCtr < 24; hrCtr++) {
			if (currentRoomBooking < roomBookings.size() &&
				...
				acc += displayBooking(roomBookings.get(currentRoomBooking), null);
	 ...
	 *
	 * @param file filename to use for saving
	 *
	 * @return true if file saved, false otherwise
	 */
	private boolean saveBookingsToFile(String file) {
		try {
	 ...
	 *
	 * @param file filename to use for loading data from file
	 *
	 * @return ArrayList of RoomBooking Objects
	 */
	private ArrayList<RoomBooking> loadBookingsFromFile(String file) {
	 ...
	 *
	 * @param bookingInput data to create booking
	 *
	 * @return new RoomBooking Object
	 */
	public RoomBooking makeBookingFromUserInput(BookingInput bookingInput) {
		...
		String   draftName = checkIfNullOrEmpty(bookingInput.getEventClient()).trim();
		...
		String phoneNumber = checkIfNullOrEmpty(bookingInput.getEventPhoneNum()).trim();
		...
		String organization = checkIfNull(bookingInput.getEventOrganization()).trim();
		...
		String eventCategory    = checkIfNullOrEmpty(bookingInput.getEventCategory()).trim();
		String eventDescription = checkIfNullOrEmpty(bookingInput.getEventDescription()).trim();
	 ...
	 */
	public Calendar makeCalendarFromUserInput(Calendar initCal, boolean requestHour, BookingInput bookingInput) {
		...
		if (needCal) {
			date = checkIfNullOrEmpty(bookingInput.getEventDate());
			try {
		...
		if (requestHour) {
			String time = checkIfNullOrEmpty((needCal ? bookingInput.getEventSTime() : bookingInput.getEventETime()));
	 ...
	 */
	public RoomBooking findBooking(Calendar calendar) {
